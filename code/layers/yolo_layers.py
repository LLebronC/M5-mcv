...
# Works only in TF
class YOLODeconvolution2D(Layer):

    def __init__(self, nb_filter, nb_row, nb_col, batch_size = 16,
                 init='glorot_uniform', activation=None, weights=None,
                 border_mode='valid', subsample=(1, 1), dim_ordering='default',
                 W_regularizer=None, b_regularizer=None, activity_regularizer=None,
                 W_constraint=None, b_constraint=None,
        ...
        if dim_ordering == 'default':
            dim_ordering = K.image_dim_ordering()
        if border_mode not in {'valid', 'same', 'full'}:
            raise ValueError('Invalid border mode for YOLODeconvolution2D:', border_mode)
        self.nb_filter = nb_filter
        self.nb_row = nb_row
        self.nb_col = nb_col
        ...
        self.initial_weights = weights
        self.uses_learning_phase = True
        super(YOLODeconvolution2D, self).__init__(**kwargs)
    ...
    def call(self, x, mask=None):        
        ...
        if self.dim_ordering == 'th':
            outputShape = (self.batch_size, 3, rows, cols) # 32 = input_shape[0]
        elif self.dim_ordering == 'tf':
        ...
        if self.dim_ordering == 'th':
          ...
          output = tf.image.resize_nearest_neighbor(x, (new_height, new_width))
        else:
...
class concat(Layer):
    ...
    def call(self, data, mask=None):
        return K.concatenate([data[0], data[1]],1)
    
    def get_output_shape_for(self, input_shape):
        return (input_shape[0][0], input_shape[0][1]*2, input_shape[0][2], input_shape[0][3])
