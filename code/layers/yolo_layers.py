...
# Works only in TF
class YOLOConvolution2DFirstLayer(Layer):
    ...
    def __init__(self, nb_filter, nb_row, nb_col,
        ...
        self.initial_weights = weights
        self.uses_learning_phase = True
        super(YOLOConvolution2DFirstLayer, self).__init__(**kwargs)

    def build(self, input_shape):
        if self.dim_ordering == 'th':
            stack_size = input_shape[1]
            self.W_shape = (self.nb_filter/2, stack_size, self.nb_row, self.nb_col)
            self.W_shape_dec = (stack_size, stack_size, self.nb_row, self.nb_col)
        elif self.dim_ordering == 'tf':
            stack_size = input_shape[3]
            self.W_shape = (self.nb_row, self.nb_col, stack_size, self.nb_filter/2)
            self.W_shape_dec = (self.nb_row, self.nb_col, stack_size, stack_size)
        else:
            raise ValueError('Invalid dim_ordering:', self.dim_ordering)
        self.W = self.add_weight(self.W_shape,
                                 initializer=functools.partial(self.init,
        ...
        self.W_dec = self.add_weight(self.W_shape_dec,
                                 initializer=functools.partial(self.init,
                                                               dim_ordering=self.dim_ordering),
                                 name='{}_W'.format(self.name),
                                 regularizer=self.W_regularizer,
    ...
    def get_output_dec_shape_for(self, input_shape):
        if self.dim_ordering == 'th':
            rows = input_shape[2]
            cols = input_shape[3]
        elif self.dim_ordering == 'tf':
        ...
        if self.dim_ordering == 'th':
            return (input_shape[0], 3, rows, cols)
        elif self.dim_ordering == 'tf':
            return (input_shape[0], rows, cols, 3)
    ...
    def call(self, x, mask=None):        
        ...
        if self.dim_ordering == 'th':
            outputShape = (input_shape[0], 3, rows, cols) # 32 = input_shape[0]
        elif self.dim_ordering == 'tf':
            outputShape = (input_shape[0], rows, cols, 3)
            
        print 'outputShape :', outputShape
        
        output_tmp = K.deconv2d(x, self.W_dec, output_shape=outputShape,                            
                            border_mode='valid')                            
                                              
        output_tmp1 = K.conv2d(x, self.W, strides=self.subsample,
                          border_mode=self.border_mode,
                          dim_ordering=self.dim_ordering,
                          filter_shape=self.W_shape)
                          
        output_tmp2 = K.conv2d(output_tmp, self.W, strides=self.subsample,
                          border_mode=self.border_mode,
                          dim_ordering=self.dim_ordering,
                          filter_shape=self.W_shape)                          
        ...
        output_tmp3 = K.pool2d(output_tmp2, pool_size=(2, 2), strides = (2,2))
       
        print 'output_tmp3: '                       
        print output_tmp3 

        output = tf.concat([output_tmp1, output_tmp3], axis=1)
    ...
    def get_config(self):
        config = {'nb_filter': self.nb_filter,
                  ...
                  'beta_regularizer': self.beta_regularizer.get_config() if self.beta_regularizer else None,
                  'momentum': self.momentum}
        base_config = super(YOLOConvolution2DFirstLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
